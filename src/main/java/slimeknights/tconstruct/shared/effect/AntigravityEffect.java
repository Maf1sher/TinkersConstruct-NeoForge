...
public class AntigravityEffect extends TinkerEffect {
  ...
  /** Gets this effect as a holder for use with hasEffect etc. */
  private Holder<MobEffect> asHolder() {
  ...
  private void onLivingJump(LivingJumpEvent event) {
    ...
    if (entity.hasEffect(asHolder()) && entity.getAttributeValue(Attributes.GRAVITY) < 0) {
  ...
  public boolean antigravityJump(Player player) {
    ...
    if (player.verticalCollision && !player.verticalCollisionBelow && !player.isInWaterOrBubble()
      && player.hasEffect(asHolder()) && player.getAttributeValue(Attributes.GRAVITY) < 0 && !player.onClimbable()) {
