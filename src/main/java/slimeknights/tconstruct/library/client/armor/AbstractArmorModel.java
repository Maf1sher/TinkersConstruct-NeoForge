...
public abstract class AbstractArmorModel extends Model {
  ...
  public static void renderColored(Model model, PoseStack matrices, VertexConsumer buffer, int packedLightIn, int packedOverlayIn, int color, int tintColor) {
    int finalColor;
    if (color != -1) {
      finalColor = FastColor.ARGB32.multiply(color, tintColor);
    } else {
      finalColor = tintColor;
    }
    model.renderToBuffer(matrices, buffer, packedLightIn, packedOverlayIn, finalColor);
  }
