...
public class ToolBuildingRecipeBuilder extends AbstractRecipeBuilder<ToolBuildingRecipeBuilder> {
  ...
  public void save(RecipeOutput consumerIn, ResourceLocation id) {
    @Nullable AdvancementHolder advancement = this.buildOptionalAdvancement(consumerIn, id, "parts");
    if (tippedModifier != null) {
      ...
      saveRecipe(consumerIn, id, new TippedToolTransformRecipe(id, group, output, layoutSlot, extraRequirements.get(0), extraMaterials, tippedModifier), advancement);
    } else {
      saveRecipe(consumerIn, id, new ToolBuildingRecipe(id, group, output, outputSize, layoutSlot, extraRequirements, partsOverride, extraMaterials), advancement);
    }
  }
}
