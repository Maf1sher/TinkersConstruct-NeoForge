...
public class IncrementalModifierRecipe extends AbstractModifierRecipe {
  ...
  public void updateInputs(LazyToolStack result, IMutableTinkerStationContainer inv, boolean isServer) {
    ...
    ModifierId modifier = this.result.getModifierId();
  ...
  protected List<List<ItemStack>> getInputs() {
    if (slotCache == null) {
      ...
      Lazy<List<ItemStack>> fullSize = Lazy.of(() -> items.stream().map(stack -> stack.copyWithCount(maxStackSize)).collect(Collectors.<ItemStack>toList()));
      ...
      if (needed > 0) {
        ...
        builder.add(items.stream().map(stack -> stack.copyWithCount(remaining)).collect(Collectors.<ItemStack>toList()));
      }
