...
public class SeveringRecipeBuilder extends AbstractRecipeBuilder<SeveringRecipeBuilder> {
  ...
  public void save(RecipeOutput consumer, ResourceLocation id) {
    @Nullable AdvancementHolder advancement = this.buildOptionalAdvancement(consumer, id, "severing");
    if (childOutput != null) {
      saveRecipe(consumer, id, new AgeableSeveringRecipe(id, ingredient, output, childOutput, baseChance, lootingBonus), advancement);
    } else {
      saveRecipe(consumer, id, new SeveringRecipe(id, ingredient, output, baseChance, lootingBonus), advancement);
    }
  }
}
