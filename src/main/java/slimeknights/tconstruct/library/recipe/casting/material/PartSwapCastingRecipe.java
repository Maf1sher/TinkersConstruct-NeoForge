            ...
            .flatMap(recipe -> {
              ...
              for (ItemStack cast : casts) {
                ...
                if (index < requirements.size() && requirements.get(index).canUseMaterial(output.getId())) {
                  ...
                  net.minecraft.world.item.component.CustomData.update(net.minecraft.core.component.DataComponents.CUSTOM_DATA, input, tag -> tag.putBoolean(TooltipUtil.KEY_DISPLAY, true));
                  inputs.add(input);
                }
              }
