...
public class TagDifferencePresentCondition<T> implements ICondition {
  ...
  public static final MapCodec<TagDifferencePresentCondition<?>> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
    ResourceLocation.CODEC.optionalFieldOf("registry", Registries.ITEM.location())
      .forGetter(c -> c.base.registry().location()),
    ResourceLocation.CODEC.fieldOf("base")
      .forGetter(c -> c.base.location()),
    ResourceLocation.CODEC.listOf().fieldOf("subtracted")
      .forGetter(c -> c.subtracted.stream().map(TagKey::location).toList())
  ).apply(builder, (registry, base, subtracted) -> {
    ResourceKey<Registry<Object>> regKey = ResourceKey.createRegistryKey(registry);
    TagKey<Object> baseKey = TagKey.create(regKey, base);
    List<TagKey<Object>> subtractedKeys = subtracted.stream().map(loc -> TagKey.create(regKey, loc)).toList();
    return new TagDifferencePresentCondition(baseKey, subtractedKeys);
  }));
