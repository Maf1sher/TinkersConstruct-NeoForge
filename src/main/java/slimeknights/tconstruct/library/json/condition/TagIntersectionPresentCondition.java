...
public class TagIntersectionPresentCondition<T> implements ICondition {
  ...
  public static final MapCodec<TagIntersectionPresentCondition<?>> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
    ResourceLocation.CODEC.optionalFieldOf("registry", Registries.ITEM.location())
      .forGetter(c -> c.names.get(0).registry().location()),
    ResourceLocation.CODEC.listOf().fieldOf("tags")
      .forGetter(c -> c.names.stream().map(TagKey::location).toList())
  ).apply(builder, (registry, tags) -> {
    ResourceKey<Registry<Object>> regKey = ResourceKey.createRegistryKey(registry);
    List<TagKey<Object>> tagKeys = tags.stream().map(loc -> TagKey.create(regKey, loc)).toList();
    return new TagIntersectionPresentCondition(tagKeys);
  }));
