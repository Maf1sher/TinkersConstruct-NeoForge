...
public class ModifierBonusLootFunction extends LootItemConditionalFunction {
  /** Map of formula types by ID, matching ApplyBonusCount's approach */
  private static final Map<ResourceLocation, FormulaType> FORMULAS = Stream.of(
    BinomialWithBonusCount.TYPE, OreDrops.TYPE, UniformBonusCount.TYPE
  ).collect(Collectors.toMap(FormulaType::id, Function.identity()));

  private static final Codec<FormulaType> FORMULA_TYPE_CODEC = ResourceLocation.CODEC
    .comapFlatMap(
      id -> {
  ...
  public static final MapCodec<ModifierBonusLootFunction> CODEC = RecordCodecBuilder.mapCodec(
    instance -> commonFields(instance)
      .and(instance.group(
        ResourceLocation.CODEC.fieldOf("modifier").forGetter(f -> f.modifier.location()),
        FORMULA_CODEC.forGetter(f -> f.formula),
        Codec.BOOL.optionalFieldOf("include_base", true).forGetter(f -> f.includeBase)
      ))
      .apply(instance, (conditions, modifierId, formula, includeBase) ->
        new ModifierBonusLootFunction(conditions, new ModifierId(modifierId), formula, includeBase))
  );
