...
public class ThrownTool extends ThrownTrident implements ToolProjectile {
  ...
  public ThrownTool(Level level, LivingEntity shooter, ItemStack stack, float charge, float multiplier, float waterInertia) {
    ...
    // set the pickup item stack (replaces old tridentItem field)
  ...
  /** Gets the tool item stack (replaces old tridentItem field access) */
  ...
  private void updateFromStack() {
    ...
    this.entityData.set(STACK, toolItem);
    this.entityData.set(ID_LOYALTY, (byte) ModifierUtil.getVolatileInt(toolItem, LOYALTY));
    this.entityData.set(ID_FOIL, ModifierUtil.checkVolatileFlag(toolItem, ModifiableItem.SHINY));
    this.noDespawn = ModifierUtil.checkVolatileFlag(toolItem, IndestructibleItemEntity.INDESTRUCTIBLE_ENTITY);
    if (!level().isClientSide) {
      this.magnet = ModifierUtil.getVolatileInt(toolItem, MAGNET);
    }
  }
  ...
  public void onRelease(LivingEntity entity, ModDataNBT arrowData) {
    ...
    for (ModifierEntry entry : tool.getModifierList()) {
      entry.getHook(ModifierHooks.PROJECTILE_THROWN).onProjectileShoot(tool, entry, entity, toolItem, this, null, arrowData, true);
    }
    this.tasks = ScheduledProjectileTaskModifierHook.createSchedule(tool, toolItem, this, null, arrowData);
  }
  ...
  /** Checks if the tool has channeling modifier. In 1.21, isChanneling() was removed from ThrownTrident. */
  public boolean isChanneling() {
    ItemStack toolItem = getToolItem();
    return !toolItem.isEmpty() && getTool().getModifiers().getLevel(ModifierIds.channeling) > 0;
  }
  ...
  public Component getDisplayName() {
    ...
    IToolStackView tool = getTool();
    return ToolNameHook.getName(tool.getDefinition(), toolItem, tool);
  }
  ...
  public void tickDespawn() {
    ...
    if (pickup != Pickup.ALLOWED || toolItem.isEmpty()) {
      life += 1;
      if (life >= 1200) {
        this.discard();
      }
  ...
  protected void onBelowWorld() {
    ...
    if (pickup == Pickup.ALLOWED && this.entityData.get(ID_LOYALTY) != 0) {
      ...
      ItemStack toolItem = getToolItem();
      if (!toolItem.isEmpty()) {
        ToolDamageUtil.damage(getTool(), 1, getOwner() instanceof LivingEntity l ? l : null, toolItem);
      }
    } else {
      super.onBelowWorld();
    }
  ...
  private IToolStackView getTool() {
    if (tool == null) {
      tool = ToolStack.from(getToolItem());
    }
    return tool;
  }
  ...
  public void tick() {
    ...
    if (!dealtDamage && inGroundTime > 4) {
      // we don't damage the tool on throw, so instead damage it when it hits a block or an entity
      if (!toolItem.isEmpty() && !level().isClientSide) {
        ToolDamageUtil.damage(getTool(), 1, getOwner() instanceof LivingEntity l ? l : null, toolItem);
        ...
        this.entityData.set(STACK, toolItem, true);
      }
      dealtDamage = true;
    }
    super.tick();
    ...
    if (!tasks.isEmpty() && !toolItem.isEmpty()) {
      ScheduledProjectileTaskModifierHook.checkSchedule(getTool(), toolItem, this, null, tasks);
    }
  }
  ...
  protected void onHitEntity(EntityHitResult pResult) {
    ...
    if (!toolItem.isEmpty() && this.getOwner() instanceof LivingEntity owner) {
      ...
      if (ToolAttackUtil.canPerformAttack(tool) && ToolAttackUtil.isAttackable(owner, target)) {
        ...
        if (EntityInteractionModifierHook.meleeDisabled(tool)) {
          ...
          if (notSelf) {
            owner.setItemInHand(InteractionHand.OFF_HAND, toolItem);
          }
  ...
  protected void onHitBlock(BlockHitResult result) {
    ...
    if (!hitBlock) {
      ...
      if (!dealtDamage && !toolItem.isEmpty() && toolItem.is(TinkerTags.Items.HARVEST) && this.getOwner() instanceof ServerPlayer owner) {
        ...
        if (!tool.isBroken()) {
          ...
          if (hardness != -1 && IsEffectiveToolHook.isEffective(tool, state)) {
            ...
            if (miningSpeed > 1.5 * hardness) {
              ...
              owner.setItemInHand(InteractionHand.MAIN_HAND, toolItem);
              int harvested = ToolHarvestLogic.runBlockBreak(toolItem, tool, state, pos, result.getDirection(), owner, this);
  ...
  public void readAdditionalSaveData(CompoundTag tag) {
    ...
    // In 1.21, the item is stored as "item" by AbstractArrow, not "Trident"
    if (!getToolItem().isEmpty()) {
      updateFromStack();
    }
