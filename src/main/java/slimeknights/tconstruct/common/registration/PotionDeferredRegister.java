...
public class PotionDeferredRegister extends DeferredRegisterWrapper<Potion> {
  ...
  public Builder registerTypes(String name, Holder<? extends MobEffect> effect, int duration, int amplifier) {
    return new Builder(name, (Holder<MobEffect>) (Holder<?>) effect, duration, amplifier);
  }

  /** Registers a group of potions with the same effect */
  public Builder registerTypes(DeferredHolder<MobEffect, ? extends MobEffect> effect, int duration, int amplifier) {
    return registerTypes(effect.getId().getPath(), effect, duration, amplifier);
  }
