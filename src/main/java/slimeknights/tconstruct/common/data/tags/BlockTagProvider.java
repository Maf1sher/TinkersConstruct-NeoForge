...
public class BlockTagProvider extends BlockTagsProvider {
  ...
  private void addCommon() {
    ...
    addGlass(TinkerCommons.clearStainedGlass, "glass_blocks/", tag(Tags.Blocks.GLASS_BLOCKS));
  ...
  private void addHarvest() {
    ...
    for (DirtType dirt : DirtType.values()) {
      for (FoliageType grass : FoliageType.values()) {
        ...
        Tiers tier;
        int dirtLevel = getTierLevel(dirtTier);
        int grassLevel = getTierLevel(grassTier);
        if (dirtLevel == grassLevel) {
          tier = dirtTier.ordinal() > grassTier.ordinal() ? dirtTier : grassTier;
        } else {
          tier = dirtLevel > grassLevel ? dirtTier : grassTier;
        }
    ...
    tagBlocks(MINEABLE_MELTING_BLACKLIST, TinkerSmeltery.searedMelter, TinkerSmeltery.smelteryController, TinkerSmeltery.foundryController, TinkerSmeltery.searedLantern, TinkerSmeltery.scorchedLantern, TinkerSmeltery.searedFluidCannon, TinkerSmeltery.scorchedFluidCannon, TinkerSmeltery.searedCastingTank, TinkerSmeltery.scorchedProxyTank);
  ...
  /** Gets the harvest level for a tier, replacing the removed Tiers.getLevel() in 1.21 */
  private static int getTierLevel(Tiers tier) {
    return switch (tier) {
      case WOOD -> 0;
      case STONE -> 1;
      case IRON -> 2;
      case DIAMOND -> 3;
      case NETHERITE -> 4;
      default -> 0; // GOLD has level 0 in vanilla
    };
  }
