...
public class BlockLootTableProvider extends BlockLootSubProvider {
  ...
  private final LootItemCondition.Builder SHEARS = CanItemPerformAbility.canItemPerformAbility(ItemAbilities.SHEARS_DIG);
  private final LootItemCondition.Builder SILK_TOUCH_OR_SHEARS = SHEARS.or(hasSilkTouch());
  ...
  private LootTable.Builder dropSapling(Block leaves, Block sapling, float... fortune) {
    ...
    return droppingSilkOrShears(leaves, applyExplosionCondition(leaves, LootItem.lootTableItem(sapling))
      .when(BonusLevelTableCondition.bonusLevelFlatChance(enchantments.getOrThrow(Enchantments.FORTUNE), fortune)));
  }
      ...
      return builder.withPool(
        LootPool.lootPool().setRolls(ConstantValue.exactly(1))
                ...
                .add(applyExplosionCondition(leaves, LootItem.lootTableItem(TinkerCommons.slimeball.get(slime)))
                       .when(BonusLevelTableCondition.bonusLevelFlatChance(enchantments.getOrThrow(Enchantments.FORTUNE), 1 / 50f, 1 / 45f, 1 / 40f, 1 / 30f, 1 / 20f))));
    }
    return builder;
  }
  ...
  /** Helper to get the shears-or-silk-touch negated condition (replaces old HAS_NO_SHEARS_OR_SILK_TOUCH) */
  private LootItemCondition.Builder doesNotHaveShearsOrSilkTouch() {
    return SILK_TOUCH_OR_SHEARS.invert();
  }
}
