...
public final class TinkerFluids extends TinkerModule {
  public TinkerFluids() {
    ...
    net.neoforged.neoforge.common.NeoForge.EVENT_BUS.addListener(TinkerFluids::registerBrewingRecipes);
  }
  ...
  private static void registerBrewingRecipes(net.neoforged.neoforge.event.brewing.RegisterBrewingRecipesEvent event) {
  ...
  private static void addTabItems(ItemDisplayParameters itemDisplayParameters, CreativeModeTab.Output output) {
    ...
    BuiltInRegistries.POTION.holders().filter(holder -> {
      return !holder.is(Potions.WATER.unwrapKey().orElseThrow());
